...
def predict_energies():
    ...
    try:
        ...
        if len(fragments) != 2:
            return jsonify(
                {
                    "error": "Molecule string must contain exactly 2 fragments separated by --"
                }
            ), 400
        ...
        frag_a_lines = [
            line.strip()
            for line in fragments[0].split("\n")
            if line.strip() and not line.strip().startswith("0")
        ]
        frag_b_lines = [
            line.strip()
            for line in fragments[1].split("\n")
            if line.strip() and not line.strip().startswith("0")
        ]
        ...
        interaction_energies = apnet_pt.pretrained_models.apnet2_model_predict(
            [mol], compile=False, batch_size=1, ap2_fused=True
        )
        ...
        # Generate XYZ format string for 3Dmol.js
        symbols = mol.symbols
        xyz_lines = [str(len(symbols))]
        xyz_lines.append("")  # Comment line
        ...
        response = {
            ...
            "atom_counts": {"molecule_a": atom_count_a, "molecule_b": atom_count_b},
